...
**File IDs**: Each upload gets a UUIDv4, and returned URLs include the original
filename and extension (e.g. `<host>/<uuid>/<original.ext>`), so names are
preserved without additional settings.
...
The provided `docker-compose.yml` builds the `airliftd` image, persists the
application data in `./data/airliftd`, and publishes port 60606 on the host:

```
$ docker compose up -d --build
```

After the container starts, visit `http://<host>:60606/-/config` to set a
password and the Base URL. If you run a reverse proxy (such as HAProxy on the
host or another machine) for TLS termination, point it at port 60606 and
forward the `Host`, `X-Forwarded-For`, and `X-Forwarded-Proto` headers so
Airlift can construct proper external URLs. Everything the server generates
(config, uploads, thumbnails, sessions) lives in `./data/airliftd`, so backing
up or migrating the instance is as simple as copying that directory.

A simple `GET /health` endpoint is also available for load balancer health
checks and returns `{"status":"ok"}` with HTTP 200 to indicate readiness.
